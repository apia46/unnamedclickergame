...
var cyyanthingsalltime = float(0)
...
func processcyyan(delta):
	cyyanthings += %normal.persecondactual*delta / thingspercyyan
...
func _gainclick():
	cyyanthings -= GAINCLICKCOST
	gainclick = true

func _automatedpassive():
	cyyanthings -= AUTOMATEDPASSIVECOST
	automatedpassive = true
	_update_automatedpassive()

func _timeplayed():
	cyyanthings -= TIMEPLAYEDCOST
	timeplayed = true

func _achievsquared():
	cyyanthings -= ACHIEVSQUAREDCOST
	achievsquared = true
	achiev._update_ach()
...
func _magentermechanic():
	...
	cyyanthings -= MAGENTERMECHANICCOST
...
func _update_per_frame():
	%cyyanthings.text = ("you have " + format.number(cyyanthings, 0) + " cyyan things,\ngiving you x" + format.number(cyyanmultiply) + " thing generation")
	...
	%magentermechanic.visible = cyyanthingsalltime > MAGENTERMECHANICCOST
	if !magentermechanic:
