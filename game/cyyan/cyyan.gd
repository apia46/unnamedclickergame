...
@onready var milestones = $"cont/topside/margins/milestones"
...
func updateText():
	$"cont/textCont/cyyanLabel".text = "You have "+cyyan.F("cyyan thing")
	$"cont/textCont/cyyanPerSecondLabel".text = "You are gaining "+cyyanPerSecond.F("cyyan thing")+" per second"
	var until = thingsPerCyyan.Mul(1-fmod(cyyan.ToFloat(),1))
	$"cont/textCont/perLabel".text = "+1 cyyan thing per "+thingsPerCyyan.F("thing")+" you get"+(", "+until.F()+" until next" if cyyan.LessThan(1e16) else "")
